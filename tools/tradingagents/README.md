...
研究失敗不自動再次花 API 預算；重新研究要由使用者手動新開案。成功後的返工重用同一份 SHA-256 原始證據，不重新查詢。原始 JSON 隨員工草稿交主管審查，只有整案 PASS 才可正式下載。PASS 的成果頁會從這份已校驗 JSON 顯示 OHLCV K 線、成交量、截止日期、資料來源及限制；不從模型文字補行情。正式成果沿用文件／資料等分類。
